  ...
  <div class="warehouse-page">
    ...
    <el-card class="page-card" shadow="never">
      <el-tabs v-model="tab">
        ...
        <el-empty v-if="!loading && warehouses.length === 0" description="暂无数据" />
        </el-tab-pane>
        ...
        <el-empty v-if="!loading && suppliers.length === 0" description="暂无数据" />
        </el-tab-pane>

        <el-tab-pane label="采购单" name="purchase">
          <el-table :data="purchaseOrders" stripe v-loading="loading">
        ...
        <el-empty v-if="!loading && purchaseOrders.length === 0" description="暂无数据" />
        </el-tab-pane>

        <el-tab-pane label="出库单" name="outbounds">
          <el-table :data="outbounds" stripe v-loading="loading">
            <el-table-column type="expand">
              <template #default="{ row }">
                <el-table :data="row.lines" size="small" class="line-table">
            ...
            <el-table-column label="创建时间" min-width="180"><template #default="{ row }">{{ formatDateTime(row.createdAt) }}</template></el-table-column>
        ...
        <el-empty v-if="!loading && outbounds.length === 0" description="暂无数据" />
        </el-tab-pane>

        <el-tab-pane label="在途" name="transit">
          <el-table :data="inTransit" stripe v-loading="loading">
            <el-table-column prop="outboundId" label="出库单" width="100" />
            ...
            <el-table-column label="发运时间" min-width="180"><template #default="{ row }">{{ formatDateTime(row.createdAt) }}</template></el-table-column>
          </el-table>
        <el-empty v-if="!loading && inTransit.length === 0" description="暂无数据" />
        </el-tab-pane>
        ...
        <el-empty v-if="!loading && inventory.length === 0" description="暂无数据" />
        </el-tab-pane>
        ...
        <el-tab-pane label="库存流水" name="movements">
          <el-table :data="movements" stripe>
            ...
            <el-table-column label="时间" min-width="180"><template #default="{ row }">{{ formatDateTime(row.createdAt) }}</template></el-table-column>
          </el-table>
        <el-empty v-if="!loading && movements.length === 0" description="暂无数据" />
        </el-tab-pane>
      </el-tabs>
    </el-card>
...
<style scoped>
.warehouse-page{display:grid;gap:16px}.hero-card{display:flex;justify-content:space-between;gap:24px;align-items:center;padding:24px 28px;border-radius:18px;color:#fff;background:linear-gradient(135deg,#172554 0%,#1d4ed8 58%,#0ea5e9 100%);box-shadow:0 18px 40px rgba(29,78,216,.2)}.eyebrow{margin:0 0 6px;font-size:11px;letter-spacing:.18em;opacity:.7}.hero-card h2{margin:0;font-size:26px}.hero-copy{margin:8px 0 0;opacity:.82}.hero-actions{display:flex;gap:10px;flex-wrap:wrap}.page-card{border:0;border-radius:16px}.line-table{margin:8px 44px;width:calc(100% - 88px)}.muted,.field-hint{color:var(--layout-muted);font-size:13px}.positive{color:#059669;font-weight:700}.negative{color:#dc2626;font-weight:700}.form-grid{display:grid;grid-template-columns:1fr 1fr;gap:0 16px}.section-title{display:flex;justify-content:space-between;align-items:center;margin:8px 0 12px;font-weight:700}.purchase-line-card{padding:16px;margin-bottom:14px;border:1px solid var(--layout-border);border-radius:12px;background:var(--el-fill-color-light)}.line-card-head{display:flex;justify-content:space-between;align-items:center;margin-bottom:14px}.line-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.line-field{display:grid;gap:7px;color:var(--layout-muted);font-size:13px}.line-field :deep(.el-input-number){width:100%}.native-date{width:100%;height:32px;padding:0 10px;border:1px solid var(--layout-border);border-radius:4px;color:var(--layout-text);background:var(--layout-card);box-sizing:border-box}.native-date:focus{outline:none;border-color:var(--el-color-primary)}.receive-table{margin:14px 0}.el-select{width:100%}@media(max-width:900px){.hero-card{align-items:flex-start;flex-direction:column}.form-grid{grid-template-columns:1fr}.line-grid{grid-template-columns:1fr 1fr}}@media(max-width:620px){.line-grid{grid-template-columns:1fr}.line-card-head{align-items:flex-start}.purchase-line-card{padding:14px}}
.master-data-cell{display:grid;gap:2px;line-height:1.35}.master-data-cell strong{color:var(--layout-text);font-weight:650}.master-data-cell small{color:var(--layout-muted);font-size:11px;letter-spacing:.02em}
</style>
