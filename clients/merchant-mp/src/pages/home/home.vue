  ...
  <view>
    ...
    <view v-else>
      ...
      <view class="action-grid">
        ...
        <view class="action-card" @click="goSettlements">
          <text class="action-icon">💰</text>
          <text class="action-label">结算对账</text>
        </view>
        <view class="action-card" @click="goDisputes">
          <text class="action-icon">⚖️</text>
          <text class="action-label">争议处理</text>
        </view>
      </view>
...
function goSettlements() {
  goTab('/pages/settlements/settlements');
}

function goDisputes() {
  goTab('/pages/disputes/disputes');
}
  loading.value = true;
  error.value = '';
  try {
...
.action-card {
    cursor: pointer; background: #fff; border-radius: 16rpx; padding: 20rpx 10rpx; text-align: center; box-shadow: 0 2px 12px rgba(15,118,110,0.08); }
