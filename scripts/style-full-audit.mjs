...
function auditFile(absPath, app, globalDefinedVars, globalClassSet) {
  ...
  res.hexBareTop = Object.fromEntries(
    [...wholeStats.bare.entries()].sort((a, b) => b[1] - a[1]).slice(0, 5)
  );
  ...
  const hwInStyle =
    /\.(empty-title|empty-desc|empty-hint|empty-state-text)\s*[{,]/.test(style) &&
    !absPath.includes('empty-state');
...
for (const [appName, app] of Object.entries(APPS)) {
  ...
  for (const t of globalTexts)
    for (const m of t.matchAll(/\.([a-zA-Z][\w-]*)/g)) globalClassSet.add(m[1]);
  ...
  if (app.isAdmin) {
    ...
    registry.registered = [...routerSrc.matchAll(/@\/views\/([\w/.-]+\.vue)/g)].map(
      (m) => `views/${m[1]}`
    );
    const regSet = new Set(registry.registered);
    registry.orphans = pageFiles
      .map(rel)
      .filter((f) => !regSet.has(f.replace(/^clients\/admin-vue\/src\//, '')));
  } else {
    const pj = JSON.parse(read(join(srcDir, 'pages.json')));
    registry.globalStyle = pj.globalStyle || null;
    registry.tabBar = pj.tabBar
      ? {
          color: pj.tabBar.color,
          selectedColor: pj.tabBar.selectedColor,
          backgroundColor: pj.tabBar.backgroundColor
        }
      : null;
    const entries = [];
    for (const p of pj.pages || [])
      entries.push({ ...p, path: p.path.startsWith('pages/') ? p.path : `pages/${p.path}` });
    ...
    registry.orphans = pageFiles
      .map(rel)
      .filter((f) => !regSet.has(f.replace(/^clients\/[^/]+\/src\//, '')));
    for (const e of entries) {
      ...
      for (const k of [
        ...
        if (e[k] && (!registry.globalStyle || registry.globalStyle[k] !== e[k]))
          styleOverrides[k] = e[k];
      }
      if (Object.keys(styleOverrides).length)
        registry.pageOverrides.push({ page: e.path, ...styleOverrides });
    }
  }
...
function sumTotals(pages, others) {
  ...
  for (const f of all) {
    t.hexBare += f.hexBare;
    t.hexFallback += f.hexFallback;
    t.radiusLit += f.radiusLit;
    t.inlineStyleHex += f.inlineStyleHex;
    t.btnNative += f.btnNative;
    t.btnApp += f.btnApp;
    t.emptyCmp += f.emptyCmp;
    t.emptyHw += f.emptyHw;
    t.errCmp += f.errCmp;
    t.chartHexScript += f.chartHexScript;
  }
  return t;
}
...
function buildMarkdown(result) {
  ...
  const row = (label, get) =>
    L.push(
      `| ${label} | ${get(result.apps['admin-vue'])} | ${get(result.apps['consumer-mp'])} | ${get(result.apps['merchant-mp'])} |`
    );
  ...
  for (const [key, title] of sections) {
    ...
    if (key === 'admin-vue') {
      ...
      for (const p of app.pages) {
        L.push(
          `| ${shortFile(p.file)} | ${fmt(p.hexBare)} | ${fmt(p.hexFallback)} | ${fmt(p.radiusLit)} | ${fmt(p.inlineStyleHex)} | ${fmt(p.btnNative)} | ${p.pageCard > 0 ? '✓' : '✗'} | ${p.usesChart ? '✓' : '·'} | ${fmt(p.chartHexScript)} |`
        );
      }
    } else {
      L.push(
      ...
      for (const p of app.pages) {
        L.push(
          `| ${shortFile(p.file)} | ${fmt(p.hexBare)} | ${fmt(p.hexFallback)} | ${fmt(p.radiusLit)} | ${fmt(p.inlineStyleHex)} | ${fmt(p.btnNative)} | ${fmt(p.btnApp)} | ${fmt(p.emptyCmp)} | ${p.emptyHw ? '⚠' : '·'} | ${p.undefVars.length ? p.undefVars.join(',') : '·'} |`
        );
      }
    }
    L.push('');
  ...
  for (const [key, app] of Object.entries(result.apps)) {
    for (const f of app.others) {
      if (f.hexBare > 0 || f.radiusLit > 0 || f.btnNative > 0 || f.undefVars.length) {
        L.push(
          `| ${key} | ${shortFile(f.file)} | ${fmt(f.hexBare)} | ${fmt(f.radiusLit)} | ${fmt(f.btnNative)} | ${f.undefVars.length ? f.undefVars.join(',') : '·'} |`
        );
      }
    }
  }
  ...
  for (const [key, app] of Object.entries(result.apps)) {
    ...
    L.push(
      `- **${key}**：实际页面文件 ${app.totals.pages} 个；注册/路由引用 ${r.registered.length} 个；缺失文件 ${r.missingFiles.length}；未注册孤儿 ${r.orphans.length}${r.orphans.length ? '：' + r.orphans.join('、') : ''}`
    );
  }
  L.push('');
  for (const key of ['consumer-mp', 'merchant-mp']) {
    ...
    if (r.globalStyle) {
      L.push(
        `**${key} globalStyle**：${JSON.stringify(r.globalStyle)}；tabBar：${JSON.stringify(r.tabBar)}`
      );
  ...
  for (const [comp, apps] of Object.entries(byComp)) {
    const cell = (a) =>
      apps[a] ? (apps[a].identical ? '一致' : `⚠ 差异 ${apps[a].diffLines} 行`) : '（无本地拷贝）';
  ...
  for (const [key, app] of Object.entries(result.apps)) {
    for (const f of [...app.pages, ...app.others]) {
      if (f.ghostClasses.length)
        L.push(`- ${key} · ${shortFile(f.file)}：${f.ghostClasses.join('、')}`);
    }
  }
  L.push('');
  // 十、改造切片工单（零遗漏映射）
  ...
  L.push('');
  const list = (app, pred) =>
    result.apps[app].pages
      .filter(pred)
      .map((p) => `${shortFile(p.file)}(${p.btnNative})`)
      .join('、');
  const fileList = (app, pred) =>
    result.apps[app].pages
      .filter(pred)
      .map((p) => shortFile(p.file))
      .join('、');
  ...
  for (const key of ['consumer-mp', 'merchant-mp']) {
    const hot = result.apps[key].pages
      .filter((p) => p.hexBare >= 3)
      .sort((a, b) => b.hexBare - a.hexBare);
    L.push(
      `   - ${key}：${hot.map((p) => `${shortFile(p.file)}(${p.hexBare})`).join('、') || '无'}`
    );
  }
  L.push(`6. 字面圆角接 token（≥3 处的页面优先）：`);
  for (const key of ['consumer-mp', 'merchant-mp']) {
    const hot = result.apps[key].pages
      .filter((p) => p.radiusLit >= 3)
      .sort((a, b) => b.radiusLit - a.radiusLit);
    L.push(
      `   - ${key}：${hot.map((p) => `${shortFile(p.file)}(${p.radiusLit})`).join('、') || '无'}`
    );
  }
  L.push('');
  L.push('**P2 · 管理后台收尾**');
  L.push('');
  const adminCharts = result.apps['admin-vue'].pages.filter(
    (p) => p.usesChart || p.chartHexScript > 0
  );
  L.push(
    `1. ECharts 迁移（替代手写 SVG）：${adminCharts.map((p) => `${shortFile(p.file)}(script裸色${p.chartHexScript})`).join('、')}`
  );
  L.push(
    '   - ChartBox 保留加载/错误/空态壳，内部换 ECharts；主题色值从 CSS 变量读取，跟随暗色/主题切换；迁完删 utils/charts.ts SVG 体系。'
  );
  const adminBtn = result.apps['admin-vue'].pages
    .filter((p) => p.btnNative > 0)
    .sort((a, b) => b.btnNative - a.btnNative);
  L.push(
    `2. 原生 <button> 归一到 el-button：${adminBtn.map((p) => `${shortFile(p.file)}(${p.btnNative})`).join('、')}`
  );
  const adminRadius = result.apps['admin-vue'].pages
    .filter((p) => p.radiusLit >= 3)
    .sort((a, b) => b.radiusLit - a.radiusLit);
  L.push(
    `3. 字面圆角接 --radius-*：${adminRadius.map((p) => `${shortFile(p.file)}(${p.radiusLit})`).join('、') || '无'}（其余 1-2 处的随页面顺手改）`
  );
  const adminHot = result.apps['admin-vue'].pages
    .filter((p) => p.hexBare >= 5)
    .sort((a, b) => b.hexBare - a.hexBare);
  L.push(
    `4. 裸 hex 收敛（≥5 处优先）：${adminHot.map((p) => `${shortFile(p.file)}(${p.hexBare})`).join('、')}`
  );
  L.push(
...
writeFileSync(
  join(ROOT, `docs/style-consistency-full-audit-${DATE}.json`),
...
for (const [name, app] of Object.entries(result.apps)) {
  console.log(
    `${name}: 页面 ${app.totals.pages} 个, 其他文件 ${app.totals.otherFiles} 个, 裸hex ${app.totals.hexBare}, 字面圆角 ${app.totals.radiusLit}, 原生按钮 ${app.totals.btnNative}, 手写空状态 ${app.totals.emptyHw}, 孤儿页 ${app.registry.orphans.length}`
  );
}
