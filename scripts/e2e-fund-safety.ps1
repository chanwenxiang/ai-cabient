...
$e2eLock = Enter-E2eLock -Owner "e2e-fund-safety"
try {
    Clear-E2eDeviceBlockingSessions -DeviceId $DeviceId | Out-Null
    $auth = Get-E2eAuth -Phone $ConsumerPhone -Password $ConsumerPassword

    # Waive leftover OPEN disputes so a delayed CONFIRM cannot charge after balance reset.
    ...
    try {
        $open = Invoke-E2eApi -BaseUrl $BaseUrl -Method GET -Path "/api/v2/ops/disputes?status=OPEN&page=0&size=50" -Headers $ops
        foreach ($t in @($open.items)) {
            if (-not $t.ticketId) { continue }
            try {
                Invoke-E2eApi -BaseUrl $BaseUrl -Method POST -Path "/api/v2/ops/disputes/$($t.ticketId)/resolve" -Headers $ops -Body @{
                    resolutionType = "WAIVE"
                } | Out-Null
                Write-Host "    waived leftover dispute $($t.ticketId)"
            } catch {
                Write-Warning "waive $($t.ticketId) failed: $($_.Exception.Message)"
            }
        }
    } catch {
        Write-Warning "list OPEN disputes failed: $($_.Exception.Message)"
    }
    ...
    if (-not $SkipBalanceTest) {
        ...
        if ($result.FinalState -ne "DISPUTED") {
            throw "Expected DISPUTED (balance or recognition hold), got $($result.FinalState)"
        }
...
"@ 2>&1
        ...
        $orderCount = (docker exec ai-cabinet-postgres-1 psql -U aicabinet -d aicabinet -t -A -c `
            "SELECT COUNT(*) FROM cabinet_order WHERE session_id='$sessionId';").Trim()
        if ($orderCount -ne "0") {
            throw "Expected no order for held session, got order_count=$orderCount"
        }
