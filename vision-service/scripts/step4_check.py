...
def sim_media_key(device_id: str, user_id: int, session_id: str, camera: str, ext: str) -> str:
    from datetime import datetime
    from zoneinfo import ZoneInfo

    now = datetime.now(ZoneInfo("Asia/Shanghai"))
    date = now.strftime("%Y/%m/%d")
    safe_device = "".join(c if c.isalnum() or c in "._-" else "_" for c in device_id.strip()) or "unknown-device"
    safe_session = "".join(c if c.isalnum() or c in "._-" else "_" for c in session_id.strip()) or "session"
    cam = (camera or "top").strip().lower() or "top"
    extension = ext if ext.startswith(".") else f".{ext}"
    return f"sim/{date}/{safe_device}/user-{user_id}/{safe_session}-{cam}{extension}"
...
def main() -> int:
    ...
    # 2) minio:// single
    key = sim_media_key("step4-device", 0, f"step4-{tag}", "top", ".jpg")
    uri = upload_minio(
    ...
    top_key = sim_media_key("step4-device", 0, f"step4-{tag}", "top", ".jpg")
    side_key = sim_media_key("step4-device", 0, f"step4-{tag}", "side", ".jpg")
