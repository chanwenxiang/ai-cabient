...
export function formatConsistencyValue(
  checkType: string,
  role: 'expected' | 'actual',
  raw: string | null | undefined,
  checkKey?: string | null
): string {
  if (raw == null || String(raw).trim() === '') return '暂无';
  ...
  if (checkType === 'INVENTORY_ORPHAN_LOT' && role === 'expected' && v === '0') return '无汇总行';
  if (checkType === 'REVENUE_SPLIT_MISSING' && role === 'actual' && v === '0') return '无分账记录';
  if (checkType === 'WAREHOUSE_NEGATIVE' && role === 'expected' && v === '0') return '应 ≥ 0';
  ...
  if (/^-?\d+$/.test(v)) {
    ...
    if (checkType === 'POINTS_BALANCE' || checkType === 'POINTS_IDENTITY') return `${v} 积分`;
    if (
      checkType === 'INVENTORY_MISMATCH' ||
    ...
    if (checkType === 'COUPON_ISSUED' || checkType === 'COUPON_OVER_QUOTA') return `${v} 张`;
    return v;
  }
  ...
  if (/^[A-Z][A-Z0-9_]*$/.test(v)) return '未知';
  return v;
}
