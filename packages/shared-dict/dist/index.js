...
export function isRuntimeDictLoaded() {
    return runtimeLoaded;
}
export function setDictOverrides(map, options) {
...
const DEVICE_FAULT_ISSUE_ALIASES = {
    DOOR_WONT_OPEN: 'DOOR_OPEN',
    DOOR_WONT_CLOSE: 'DOOR_CLOSE',
    DOOR: 'DOOR_OPEN'
};
function faultIssueLabel(code) {
...
export function auditActionLabel(action) {
    ...
    if (hit)
        return hit;
    const ops = DICT.ops_exception_action[action];
    if (ops)
        return ops;
...
export function formatExceptionDetail(detail) {
    if (!detail)
        return '暂无';
    ...
    if (faultLegacy) {
        ...
        text = text.replace(/问题类型\s*=\s*([A-Za-z_]+)/g, (_, code) => {
            return `问题类型：${faultIssueLabel(code)}`;
        });
    }
    text = text.replace(/补充说明\s*=/g, '补充说明：');
    text = text.replace(/^provider\s*=\s*(\S+)\s+confidence\s*=\s*([\d.]+)(?:\s*\|\s*(.*))?$/i, (_full, provider, conf, extra) => {
    ...
    if (/^[a-zA-Z]+\s*=\s*[A-Z0-9_]+$/.test(text)) {
        ...
        let valLabel = val;
        if (key === 'onlineStatus')
            valLabel = dictLabel('online_status', val) || val;
...
export function formatRiskEventDetail(detail) {
    if (!detail)
        return '暂无';
...
export function formatOpsActionDetail(detail) {
    ...
    text = text.replace(/([A-Za-z][A-Za-z0-9_]*)\s*=\s*([^,;]+)/g, (full, key, raw) => {
        ...
        if (k === 'targettempc')
            return `${label}：${value}℃`;
        if (k === 'onlinestatus')
            return `${label}：${dictLabel('online_status', value) || value}`;
        if (k === 'lifecycle' || k === 'lifecyclestatus') {
            return `${label}：${dictLabel('device_lifecycle', value) || value}`;
        }
        if (k === 'issuetype')
            return `${label}：${faultIssueLabel(value)}`;
        if (k === 'provider' && value.toLowerCase() === 'unknown')
            return `${label}：未知`;
        return `${label}：${value}`;
    });
