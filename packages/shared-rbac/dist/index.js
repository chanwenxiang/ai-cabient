 ...
 */
export function matchPermission(granted, code) {
    if (!code)
        return true;
    const perms = granted || [];
...
/** 权限码所属域；无法识别则 null */
export function permissionRealm(code) {
    if (!code)
        return null;
    const head = code.split(':')[0];
    if (head === PERMISSION_REALMS.ops)
        return PERMISSION_REALMS.ops;
    if (head === PERMISSION_REALMS.merchant)
        return PERMISSION_REALMS.merchant;
    return null;
}
/**
...
const TEAM_PREFIXES = ['merchant:profile:', 'merchant:users:'];
export function isMerchantPackAgnostic(permCode) {
...
function matchesPrefix(code, prefixes) {
    for (const prefix of prefixes) {
        if (code.startsWith(prefix) || code === prefix.slice(0, -1))
            return true;
    }
    return false;
}
/** @returns field / biz / team，或 null（包无关 / 未知） */
export function merchantPackForPerm(permCode) {
    if (!permCode || isMerchantPackAgnostic(permCode))
        return null;
    const code = permCode.trim().toLowerCase();
    if (matchesPrefix(code, FIELD_PREFIXES))
        return 'field';
    if (matchesPrefix(code, BIZ_PREFIXES))
        return 'biz';
    if (matchesPrefix(code, TEAM_PREFIXES))
        return 'team';
    return null;
}
