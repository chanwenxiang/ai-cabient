...
COMMENT ON TABLE idempotency_key IS '幂等性控制表：防止重复提交';
...
CREATE INDEX IF NOT EXISTS idx_rate_limit_type_key ON rate_limit_config (limit_type, limit_key);

COMMENT ON TABLE rate_limit_config IS '限流配置表：防止恶意调用';
