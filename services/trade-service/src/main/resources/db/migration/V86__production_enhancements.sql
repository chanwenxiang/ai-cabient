-- 生产增强：对账明细、OTA 灰度/CDN、设备坐标、补货路线规划（幂等，可重复执行）
...
CREATE INDEX IF NOT EXISTS idx_platform_bill_recon ON payment_platform_bill_line (recon_id);
CREATE INDEX IF NOT EXISTS idx_platform_bill_merchant ON payment_platform_bill_line (merchant_order_no);
