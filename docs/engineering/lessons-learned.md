...
| 31 | Session 超时 | 改 expire 易牵动开门/结算整类回归 | `@Scheduled` expire 与开门/关门同上帝类 | expire 四任务进 `SessionExpireService`；`@Scheduled` 只挂一处；状态变更仍走 `SessionService.transition` | `SessionExpireService`、`ScheduledTaskRegistry` |
| 32 | admin 仓库页 | 改采购弹窗易误伤盘点/出库 | 单文件 5k+ 行多业务混杂 | 采购写流进 `useWarehousePurchaseOrders` + `WarehousePurchaseDialogs`；列表/其它 tab 仍在页内，继续按域拆 | `WarehouseView.vue` |
