...
| 31 | Session 拆分 | 改 expire/开门/补货/门事件易牵动整类回归 | 调度与短事务混在上帝类 | expire→`SessionExpireService`；开门短事务→`SessionOpenService`；补货快照短事务→`SessionRestockService`；门事件/关门路径→`SessionDoorService`；状态变更仍走 `SessionService.transition`；结算仍留 `SessionService` | `SessionExpireService`、`SessionOpenService`、`SessionRestockService`、`SessionDoorService` |
| 32 | admin 仓库页 | 改一域弹窗易误伤其它域 | 单文件 5k+ 行多业务混杂 | 采购/盘点/货位/出库/调拨写流分别进 composable+Dialogs；仓库/供应商/付款/其它入库进 `useWarehouseEntityDialogs`；tab 加载进 `useWarehouseTabLoader`；页壳继续瘦 CSV/筛选 | `WarehouseView.vue` |
