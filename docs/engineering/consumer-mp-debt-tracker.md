...
| C2 | P0 | done | 无 `ConsumerEndpoints` + 无门禁；路径散落字面量 | `utils/consumer-api.ts`（大量 `/api/v2`）；`dict-runtime.ts`；`video/video.vue` 页内裸拼；仅有 `check-admin-endpoints`，无 consumer 等价 | 2026-09-25：Endpoints+门禁；2026-09-26 C2b：auth/account/payment/orders/sessions/disputes 迁入 `ConsumerEndpoints`；余设备/会员/营销 → C2c |
...
| C12 | P3 | done | `consumer-api` 上帝模块 + 页面外裸 URL | `consumer-api.ts` ~886 行；`video.vue` 绕开 API 层 | 2026-09-26：首刀 `consumerOrderVideoUrl` + `normalizeMediaUrl`（对齐 M12）；页内禁拼 base；api 面拆分 → C12b/C2b |
...
```
C6b / C5b / C2c / C12b；C10b（easycom 直指 package）与 M9 对齐（mp 风险知情延后）；C9 保持 deferred
```
