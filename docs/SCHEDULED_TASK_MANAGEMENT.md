...
备注支持「用途模板」下拉（设备巡检/资金结算/对账/库存仓储/交易订单/营销优惠/运维监控/其他），
选择即填充统一文案。模板与措辞规范见 [ADMIN_QUERY_CONVENTIONS.md](ADMIN_QUERY_CONVENTIONS.md)。

另有「设备商品 → 设备可用性」页面展示 KPI：默认当天实时口径，选择日期后查询该日期快照
（无快照时按当天口径实时计算）。
