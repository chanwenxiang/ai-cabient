...
- 缺：**Alertmanager 未部署**（业务级告警规则无人消费 —— 09-18 逐条复核精确化：`infra/prometheus/alert_rules.yml` 现 **22 条**规则，`prometheus-full.yml:5` / `prometheus.compose.yml:5` 确在 `rule_files` 里加载，**但三份 prometheus 配置都无 `alerting:` 段、全仓无 alertmanager service** ⇒ 规则**会评估、无人被通知**；Grafana 侧虽有 contact point（email → ops@aicabinet.local）与 notification policy，但 **alert rule 数为 0**、且 compose 未配 `GF_SMTP_*` ⇒ 邮件也发不出去，**两条链路末端都是断的**）。🔴 **09-18 补充（飞书口径）**：本方通知渠道是**飞书**，而 **Alertmanager 与 Grafana 都没有原生飞书 receiver**（Alertmanager 有 `wechat`/`webhook` 等、Grafana 有 `dingding`/`wecom`/`webhook`，唯独没有飞书）⇒ 这两条链路**除部署 alertmanager 外还必须有一个 webhook 转换桥**（把 Alertmanager/Grafana 的 JSON 翻成飞书的 `msg_type`/`content`）；**应用内告警链路（`OpsAlertDispatcher`）已原生支持飞书**，是本仓当前**唯一不需要额外组件**的告警出口。另：无日志聚合（Loki/ELK）、无 APM/错误追踪（OTLP exporter 无接收端）、边缘端无 metrics 上报。
...
| 项 | 内容 | 对应缺口 |
|----|------|----------|
| P0-1 | **端侧识别落地**：移远 OpenVending 阶段 A（`/internal/v1/vision/edge-results` 端侧结果接入结算）→ B（准确率监控/边缘盒监控）→ C（试点灰度）；≥1000 次拿放组合测试；低置信→人工审核规则定稿。✅ **09-19 第三十一轮：阶段 A 的入站端点已落地** —— `POST /internal/v1/vision/edge-results`（复用既有结算链路，与 Kafka 通道同一方法；新增「明确受理回执」与 `modelVersion` 必填的入口 fail-closed）。真实栈 e2e **16/16**（含 4 条负对照）+ A/B 漂移 8→2/5 例真红，证据 `docs/evidence/2026-09-19-p0-1-vision-result-ingest/`。⚠️ 仍剩：**接移远 SDK/协议本体**（外部依赖）、阶段 B（准确率/边缘盒监控）、阶段 C（灰度参数）。✅ **09-19 第三十三轮：`recognition_result` 落库已收口（本行原记的「无任何写入方，建议独立立项」已消项）** —— 该表自 `V1` 起有表无写者（Kafka 与 HTTP 两通道都不落库、线上 0 行），本批在结算侧识别收敛处（`SettlementRecognitionService#processRecognitionResultUnlocked`）补**唯一写入点**，一处覆盖同步关门/异步回调/开发上传三条路径；与结算**同事务**（`recognition_result.session_id` 外键会让独立事务与外层 `FOR UPDATE` 互锁，已用真实 PostgreSQL 对照实验否掉 `REQUIRES_NEW`），**fail-hard 不静默**（宁让本次结算回滚并可重试，也不留「已扣款、无识别记录」的对账黑洞），`model_version` 由 `V279` 放宽 32→64。真实栈 e2e 同步 **10/10** + 异步 **25/25**，A/B 漂移 **5/5 真红**，证据 `docs/evidence/2026-09-19-recognition-result-persistence/`。⚠️ 部署侧需开 `aicabinet.vision-async.enabled=true` 才有受理窗口（否则关门即同步识别并立刻结算） | G0 生死线 |
...
| P0-6 | **可观测性收口**：部署 Alertmanager 消费既有业务规则（09-18 复核：`infra/prometheus/alert_rules.yml` 已 **22 条**，且三份 prometheus 配置**都缺 `alerting:` 段** ⇒ 部署 Alertmanager 之外**还须补 `alerting:` 指向它**，并把 dev 用 `prometheus.yml` 也补上 `rule_files`），**另需给 Grafana 侧补 alert rule 或删掉其空转的 contact point/policy**（现为 0 条规则，且无 `GF_SMTP_*`）；`infra/monitoring/alerts.yml` 那 **14 条**已**定论为未实现草稿**（指标名与实现不符 + 依赖未部署的 exporter）⇒ **不挂，加 `# gate: draft-unimplemented` 标记**，由 `check:prometheus-metric-names` 的 R1 守着；Loki 日志聚合；OTLP 接收端（Tempo/Jaeger）；业务 KPI 看板（开门成功率、关门完整率、结算时长、识别准确率、争议率、MQTT 转发失败）。⚠️ 通知渠道凭据（SMTP / 机器人 webhook）需外部提供，在此之前"链路通、消息落日志"。🔴 **09-18 补充（飞书，已落地并端到端实测）**：①**应用内链路已原生支持飞书** —— `OpsAlertDispatcher` 新增 FEISHU 渠道（`msg_type`/`content` 报文 + 可选签名 + **业务码判定**：飞书 `code`/钉钉企微 `errcode` 非 0 即判投递失败，修掉「HTTP 200 即算已发送」的假绿），配置键 `ops.alert.feishu_webhook` / `ops.alert.feishu_sign_secret` 在运营台「系统 → 告警规则 → 告警渠道」填写即生效（`getValue` **直读 DB、不经缓存**，改完立即生效），**只需一个机器人 URL，不需要任何额外组件**；已重建 trade-service 镜像与运营台前端并部署，`alert-test` 三态闭环实测：可读（50 条配置中两行飞书键在）/ 可改（`PUT` code=0，回读一致）/ 可用（`FEISHU delivered=True`）；②**Prometheus/Grafana 侧另需一个 webhook 转换桥**（两者都无原生飞书 receiver，见 §2.4）—— 桥 `infra/monitoring/feishu-relay.py` + `alertmanager.yml` **已落地并在 dev 栈 `--profile alerting` 端到端实测**（Alertmanager 触发的**触发通知与恢复通知均送达真实飞书群**；过程修掉两个"看起来在工作实际不工作"的静默缺陷：单线程 HTTPServer 被 keep-alive 连接饿死致仅首条告警发出、`/last` 漏 `global` 恒空）；⚠️ **Grafana 侧未动**（contact point 仍是假 email `ops@aicabinet.local`、alert rule 为 0 条）；⚠️ **三种安全设置（关键词/签名/IP 白名单）当前均未启用，其拒绝路径未被真飞书验过**；⚠️ 实际投递用的是**测试机器人**，上线须换（URL 现存两处：`infra/.env` 与 DB `system_config`）。门槛判据：`check:ops-alert-channels`（⚠️ **链内位次会随新增门禁漂移，别引用"第 N 位"** —— 直接跑 `node scripts/check-ops-alert-channels.mjs`；另 `check:ops-alert-escalation` 守升级链） | §2.4 |
...
| F4 | **屏幕广告变现闭环** | 在素材库+投放计划+曝光上报基础上补：广告主/订单管理、CPM/CPC 计费、广告主投放报表、广告收入入账（可挂到 RevenueSplit 或独立账本）<br>⚠️ **2026-09-21 复核更正（方向已变，原文保留作基线）**：商业模式定为**流量主**而非媒体主 —— 由**腾讯**按算法投放、按月把**分成**打给我们，**不需要**广告主/投放订单/CPM-CPC 计费/广告主报表/发票那套（资质与前提取向完全不同）。本节所列「广告主/订单管理、CPM/CPC 计费、广告主投放报表」**已作废**；现行范围＝**广告位接入 + 开关 + 收益对账**，见 `docs/AD_MONETIZATION_DESIGN.md` §0 / §12。 | 盈利板块之一 |
...
| O3 | **告警升级链** | ✅ **09-19 已落盘**：聊天渠道（飞书/钉钉/企微/通用 Webhook）**一条都没真的送达**时，按**值班表**把 P0 告警升级给当班人 —— 先短信、短信不成再电话（`OpsAlertDispatcher.escalateIfNeeded`）。新增 `OnCallRoster`（ISO 星期 + `[startHour,endHour)` 半开区间，支持**跨夜班**）。三条刻意设计：①**收件人不猜** —— 解析不出值班人即 `NO_ONCALL` 不发送（fail-closed，宁可漏一次也不半夜打错人）；②**判业务码** —— SMS/PHONE 走 `deliveryError` 的 `code` 分支（网关回 HTTP 200 + 业务码非 0（余额不足/号码黑名单）= **没送达**），修掉"升级链自认为成功、值班人从没收到"；③**默认零行为变化** —— `ops.alert.escalation_enabled` seed=`FALSE` 且读取兜底 `false`。门禁 `check:ops-alert-escalation`（E1–E9：`send`/`trySend` **两条**路径都必须触发升级、默认关、不猜收件人、判业务码、两级渠道绑定各自配置键且禁硬编码地址、白名单类型必须能在源码里找到**带引号字面量**出处、键必须留在 `ops.alert.` 命名空间且不得被 `INTERNAL_ALERT_KEYS` 豁免、值班表解析 fail-closed），A/B 漂移 **11/11**（逐条注入→红→还原→sha256 一致）+ 单测 **29** 例。⚠️ 前提：真实发送需短信/外呼网关凭据（同飞书口径：链路通、未配则不打，消息落日志）。证据 `docs/evidence/2026-09-19-o3-escalation/`。原文：钉钉/企微/Webhook → 增加 P0 告警电话/SMS 升级链、值班表联动（配合争议 SLA 值班表 P1 项） |
| O4 | **协议治理** | ✅ **09-19 已落盘**：proto 已按真实协议重写（**5** 下行命令 / **4** 上行事件，头部写明权威性口径）；**实现**了此前被测试 Javadoc 与证据文档反复引用、却**根本不存在的**静态门禁 `check:edge-cloud-mqtt-contract`（proto ⟷ `CabinetConstants`/`MqttTopics` ⟷ edge Kotlin **三方对账** ＋ 内部 HTTP 端点对账），A/B **9/9** 证明会红。同批修掉 **6 处**漂移：幽灵门禁、proto 缺 4 多 2、`ALL_HEARTBEATS` 值本身是错的、`videoChunk` 死声明、`ACK` 双定义、`ALERT` 无常量。证据 `docs/evidence/2026-09-19-o4-o9/`。原文：`cabinet.proto` 与实际 JSON 消息漂移 → 以实际协议为准刷新 proto 或冻结 proto，加「协议契约测试」进 CI。**09-18 取证加严**：`proto/cabinet.proto` **不被任何构建引用**（根 pom / 各模块 pom / gradle 里 `proto\|protobuf\|protoc` 零命中）⇒ 它是**无代码生成、无消费**的纯文本契约（「没人调」形态）；实际协议是 `CabinetConstants` 的 5 个 JSON/MQTT 命令（`OPEN_DOOR`/`SET_TARGET_TEMP`/`LOCK`/`UNLOCK`/`REBOOT`），与 proto 的 3 个 oneof（open_door/force_close/ota_upgrade）**互不覆盖**。⚠️ 落地路径可行：`edge/device-simulator` 是**根 pom 的 Maven 模块**（`mvn -pl edge/device-simulator test` 可跑），契约测试有真实落点 |
...
| O9 | **Flyway 治理** | ✅ **09-19 已落盘（刻意「只向前治理」）**：种子改由 `db/seed/R__seed_*.sql`（**可重复 ⇒ 必须幂等**）承载，与结构分离；`spring.flyway.locations` 增补 `classpath:db/seed`（目录初始为空 ⇒ **零行为变化**）；新增门禁 `check:flyway-seed-separation`（新增 `V*.sql` 含数据写语句必须显式声明 `MIGRATION_KIND: schema｜backfill`，声明 `seed` 即红；`R__seed_*` 必须幂等且禁 TRUNCATE / 无条件 DELETE）＋ 自测 **10/10**；🔴 **277 个历史迁移一律不动** —— 改写已应用迁移 = checksum 漂移，而门禁是 diff-based、历史天然 grandfather。证据 `docs/evidence/2026-09-19-o4-o9/`。原文：276 个迁移 → 种子/结构分离策略，防止继续膨胀（CODEBASE_FOUNDATION P2） |
...
—— ✅ **09-23 收口（`423f65e1`）**：已落地并重建产物。🔴 机制经读 vite 源码纠正为「换成运行时
`detectScriptRel()` 特性探测 ⇒ 缺支持的浏览器走**原生 `rel=preload`**，不是丢预取」；入口 104689 → **104123B**。
取证 → `PROJECT-REFERENCE` §9 #15 / §11.76。**至此本路线图 29 条的工程侧可开工项为 0**
（剩余全为卡外部资源 或 等拍板 D2–D9）。
