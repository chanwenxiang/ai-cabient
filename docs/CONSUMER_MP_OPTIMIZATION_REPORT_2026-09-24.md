...
- 修订：2026-09-24 —— **§6 第 7 项「残留色值清单」由自报 11 处更正为实测口径**（原数偏 +1，
  且整类漏掉 3 类共 9 处）。教训：手写枚举清单必漂，现扫一遍比再抄一遍可靠。
...
7. `clients/consumer-mp/src` 内仍有偏黄绿（H≈164）残留。**⚠️ 更正**：原写「11 处 `rgba(5,150,105,·)`」，
   2026-09-24 重扫实测后订正如下（原数偏 +1，且**整类漏掉 3 类共 9 处**）：
   - `rgba(5,150,105,*)`（#059669）= **10 处**：`App.vue:289`｜`login.vue:819,884,980`｜
     `index.vue:3215,3232,3421`｜`result.vue:880`｜`orders.vue:873`｜`recharge.vue:782`
   - `rgba(4,120,87,*)`（#047857）= **2 处**：`orders.vue:807,878`
   - `static/icon-cart.svg` 的 `#047857` = **3 处**（`stroke`×1 / `fill`×2）
   - `var(--brand-deep, #065f46)` 的**回退值** = **4 处**：`coupons:343`｜`marketing:422`｜
     `orders:928`｜`nearby:350`
     ⚠️ 该回退值本身即偏黄绿，`--brand-deep` 一旦未定义就会落回旧色 —— 属**潜在**（当前未生效）
     偏差，与前 3 类不同。

   以上全部落在**阴影 / 浅底 / 图标**，**无一在落地页**；阴影色相在低透明度下目视无差，故本轮
   **未动**（避免为凑一致而引入回归）。复现：
   `rg -n "rgba\(\s*(6,\s*78,\s*59|4,\s*120,\s*87|5,\s*150,\s*105)|#(064e3b|047857|059669|065f46)" clients/consumer-mp/src`
   若要把品牌色板统一做到底，这是剩余清单。
