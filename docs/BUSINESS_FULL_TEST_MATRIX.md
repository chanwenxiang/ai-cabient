...
| ID | 步骤 | 期望 | 状态 |
|----|------|------|------|
| M-01 | `/menus` 新增目录/菜单/按钮 `F` | 保存成功；权限树可勾选 |  |
| M-02 | 角色勾选新权限 → 账号重登 | 按钮级权限生效（API 403 边界） |  |
| M-03 | 对比侧栏与 `menu.ts` | 记录差异：以 **实际路由可达 + perm** 为准；避免只改 DB 以为侧栏变了 |  |
| M-04 | 禁用菜单项 | 无权限用户不可见；超管策略符合产品约定 |  |
...
| ID | 场景 | L3 核对 | 状态 |
|----|------|---------|------|
| E-01 | 新建设备 | 列表可见；商户范围正确 |  |
| E-02 | OTA 发布 | 目标柜收到版本策略（灰度/定向） |  |
| E-03 | 识别映射 | 结算识别结果映射到正确 SKU |  |
