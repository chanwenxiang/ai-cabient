...
>
> ⚠️ **环境已变更（2026-09-21）**：`C:\Users\cwx\.wslconfig` 的 `memory` 已由 `4GB` 调到 `8GB`，
> `docker info .MemTotal` 现报 **≈7939 MB**。**下文所有 3.82 GB 的数字保留为 2026-09-19 的实测基线，未回改** ——
> 在新配额下重跑压测时，请把那些数字当作「旧环境记录」，而不是当前上限。
