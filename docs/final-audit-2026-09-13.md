...
- [x] S-P1-2 Session 拆分（`SessionExpireService` / `SessionOpenService` / `SessionRestockService` / `SessionDoorService`；SessionService 仍含结算编排）
- [x] A-P2-002 WarehouseView 拆分（采购/盘点/货位/出库/调拨写流 + 实体弹窗 + `useWarehouseTabLoader`；页约 3.7k 行，CSV/筛选壳可继续瘦）
