...
| A-P2-005 | P2 | API 抽象 | 全部视图 | ~~无集中 endpoint~~ → `AdminEndpoints` + `check:admin-endpoints`（views/composables 已清零散落 `/api/v2/ops/admin/*`；63 条试点字面量） |
...
| V-P2-5 | 中 | `tests/` | ~~覆盖 < 30%~~ → 增补 fusion / frame_extract / deepseek 离线单测（+ `fetch_catalog_classes` 修复导入） |
...
- [x] M-P1-4：`AppSheet` + `AppConfirmDialog`；补货/争议/改价/团队/我的迁入 AppSheet
...
- [x] A-P2-005（续3）：会话列表/导出/取消/录像迁入 `AdminEndpoints`；上传队列、设备详情、全局搜索、`useSessionVideo` 同步；门禁 +`/sessions`
- [x] A-P2-005（续4）：柜机列表/详情/生命周期/指令/货道/温控/二维码迁入 `AdminEndpoints`；地图/运营员/分账/补货/搜索同步；门禁 +`/devices`
- [x] A-P2-005（续5）：商户列表/创建/分账/运维配置迁入 `AdminEndpoints`；设备页与全局搜索同步；门禁 +`/merchants`
- [x] A-P2-005（续6）：商户入驻列表/保存/审批/live-hints 迁入 `AdminEndpoints`；Dashboard 待审计数同步；门禁 +`/merchant-onboarding`
- [x] A-P2-005（续7）：SKU 目录列表/创建/图片上传迁入 `AdminEndpoints`；异常/争议/打印/仓库/视觉映射同步；门禁 +`/skus`（`sku-vision` 仍待迁）
...
- [x] A-P2-005（续12）：RBAC（角色/权限/运营员/用户绑定）与部门迁入 `AdminEndpoints`；个人中心与 auth store 同步；门禁 +`/rbac`、`/departments`
- [x] A-P2-005（续13）：系统配置/字典/定时任务/审批/组织树/DevOps/审计日志迁入 `AdminEndpoints`；设备详情与审批收件箱同步；门禁 +`/system-configs`、`/dicts`、`/scheduled-tasks`、`/approvals`、`/org`、`/devops`、`/audit-logs`
- [x] A-P2-005（续14）：增长（通知/会员/积分/SKU评审/用户分析/ROI）与广告素材/活动迁入 `AdminEndpoints`；门禁 +`/growth`、`/ad`
- [x] A-P2-005（续15/收口）：用户/风控/OTA/报修/场地账单/报表/对账/一致性/视觉映射/客流/充值/设备运维事件等迁入 `AdminEndpoints`；GlobalSearch 同步；views/composables 散落 `/api/v2/ops/admin/*` 清零（门禁 63 条）
