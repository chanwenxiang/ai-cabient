...
| 全流程 UI 测试 | `browser-real-testing`（+ `playwright` skill） | `.cursor/skills/browser-real-testing/` |
| UI 设计规范 | `web-design-guidelines` + `frontend-design` | `.cursor/skills/` |

**UI 工具顺序：** Playwright MCP → Playwright CLI → 回退 `cursor-ide-browser`。SkillSpector 需 git clone（网络受限时用手动 security-scan）。
...
- 仅用 curl/日志冒充 UI 验收；Playwright 可用时跳过浏览器验证
- 未读 skill 就大段实现或宣称通过
- 提交密钥
